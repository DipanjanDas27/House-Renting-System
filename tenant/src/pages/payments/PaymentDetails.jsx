...
import { CreditCard, CheckCircle2, Clock, XCircle, Trash2, ArrowLeft, Hash } from "lucide-react"
...
const STATUS_CONFIG = {
  completed: { icon: <CheckCircle2 size={14} />, classes: "bg-green-50 text-green-700 border border-green-200" },
...
const DetailRow = ({ label, value, mono }) => (
  <div className="flex items-start justify-between gap-4 py-3.5 border-b border-beige-card/60 last:border-0">
    <span className="text-sm font-semibold text-brown-muted shrink-0">{label}</span>
    <span className={`text-sm font-bold text-brown-dark text-right ${mono ? "font-mono tracking-wide" : ""}`}>
      {value}
    </span>
  </div>
)
          ...
          <div className="px-6 pb-6 pt-2 flex gap-3">
            ...
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>
              <Button
                variant="outline"
                onClick={() => navigate(-1)}
                className="h-11 px-5 rounded-btn border-beige-card text-brown-dark font-semibold text-sm hover:bg-beige-input transition-colors duration-150 flex items-center gap-2"
              >
                <ArrowLeft size={15} />
                Go Back
              </Button>
