...
import { User, Mail, Phone, Lock, ImagePlus, UserPlus } from "lucide-react"
...
const Register = () => {
  ...
  return (
    <div className="min-h-screen bg-cream-bg flex items-center justify-center px-4 py-10 font-montserrat">
      <motion.div
      ...
      >
        <div className="bg-white rounded-card shadow-card-md px-10 py-10">
          ...
          >
            ...
            <p className="text-sm font-semibold text-brown-muted mt-1">Join Dwellio as a property owner</p>
          </motion.div>

          {/* Profile image upload */}
          <motion.div className="flex flex-col items-center mb-6" {...fieldAnim(0.18)}>
            <div className="relative">
              {preview ? (
                <img src={preview} alt="Preview" className="size-20 rounded-full object-cover border-4 border-beige-card shadow-card" />
              ) : (
                <div className="size-20 rounded-full bg-beige-card flex items-center justify-center border-4 border-white shadow-card">
                  <User size={28} className="text-brown-muted" />
                </div>
              )}
              <label
                htmlFor="profile-upload"
                className="absolute -bottom-1 -right-1 size-7 rounded-full bg-brown-dark flex items-center justify-center cursor-pointer shadow-card hover:bg-[#1a0f09] transition-colors"
              >
                <ImagePlus size={13} className="text-white" />
              </label>
              <input id="profile-upload" type="file" accept="image/png,image/jpeg,image/jpg,image/webp" className="hidden" onChange={handleFileChange} />
            </div>
            <p className="text-xs font-semibold text-brown-muted mt-2">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <motion.div className="space-y-1.5" {...fieldAnim(0.22)}>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  {...register("full_name", { required: "Name is required", minLength: { value: 2, message: "Minimum 2 characters" } })}
                />
              </div>
              {errors.full_name && <p className="text-xs font-semibold text-red-500">{errors.full_name.message}</p>}
            </motion.div>

            <motion.div className="space-y-1.5" {...fieldAnim(0.28)}>
            ...
            <motion.div className="space-y-1.5" {...fieldAnim(0.34)}>
            ...
            <motion.div className="space-y-1.5" {...fieldAnim(0.4)}>
            ...
            <motion.div className="space-y-3 pt-1" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.46, duration: 0.45 }}>
              <Button
                type="submit"
            ...
            <motion.p className="text-sm text-center text-brown-muted font-semibold pt-1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.52 }}>
              Already have an account?{" "}
              <span className="text-brown-dark font-bold hover:underline cursor-pointer" onClick={() => navigate("/login")}>
                Login
              </span>
            </motion.p>
